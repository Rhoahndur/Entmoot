...
class PDFReportGenerator:
    ...
    def _add_page_number(self, canvas_obj: canvas.Canvas, doc: SimpleDocTemplate) -> None:
        ...
        canvas_obj.drawRightString(doc.pagesize[0] - 0.75 * inch, 0.5 * inch, text)
    ...
    def _create_cover_page(self, data: ReportData) -> List[Any]:
        ...
        story.append(
            Paragraph(f"<b>Report Date:</b> {data.date.strftime('%B %d, %Y')}", info_style)
        )
    ...
    def _create_executive_summary(self, data: ReportData) -> List[Any]:
        ...
        metrics_data = [
            ...
            [
                "Total Site Area",
                f"{data.total_area_acres:.2f} acres ({data.total_area_sqm:.0f} m²)",
            ],
        ]
        ...
        if data.assets:
            ...
            metrics_data.append(
                ["Total Asset Footprint", f"{total_asset_area * 0.000247105:.2f} acres"]
            )
    ...
    def _create_site_overview(self, data: ReportData) -> List[Any]:
        ...
        if site_map:
            ...
            story.append(Paragraph("Figure 1: Site boundary and location", self.styles["Caption"]))
        ...
        if data.dem_data is not None and data.dem_bounds is not None:
            ...
            if elev_map:
                ...
                story.append(Paragraph("Figure 2: Site elevation heatmap", self.styles["Caption"]))
    ...
    def _create_constraint_analysis(self, data: ReportData) -> List[Any]:
        ...
        for i, constraint in enumerate(data.constraints[:10], 1):  # Limit to 10
            ...
            story.append(Paragraph(f"<b>{i}. {name}</b> ({area:.2f} acres)", self.styles["Normal"]))
    ...
    def _create_asset_summary(self, data: ReportData) -> List[Any]:
        ...
        if layout_map:
            ...
            story.append(Paragraph("Figure 3: Proposed asset placement", self.styles["Caption"]))
    ...
    def _create_cost_summary(self, data: ReportData) -> List[Any]:
        ...
        story.append(
            Paragraph(f"<b>Cost per Acre:</b> ${cost_per_acre:,.2f}", self.styles["CustomBody"])
        )
